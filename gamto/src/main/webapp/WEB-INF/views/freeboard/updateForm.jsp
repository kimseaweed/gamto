...
<main class="container-md bg-light px-5 pt-4 rounded shadow-sm">
		<form id="writeForm" action="/board/update" method="post" class="p-5 mb-4 bg-light rounded-3 needs-validation" novalidate>
		<input type="hidden" value="${updateForm.f_seq_number}" name="f_seq_number"/>
			<input type="hidden" name="f_writer" value="${updateForm.f_writer}" />
			<div class="mb-5">
				<h3>우리생각 | 자유롭게 작성해주세요</h3>
				<h6>작성자 : ${updateForm.f_writer}</h6>
			</div>
			<div class="mb-2">
				<select name="f_category" value="${updateForm.f_category}">
					...
					<option value="자유로운 이야기">자유로운 이야기</option>
				</select>
			</div>
			<div class="mb-3">
				<input name="f_title" class="form-control" type="text"
					id="validationCustomUsername" value="${updateForm.f_title}"
					aria-label="default input example" required>
				<div class="invalid-feedback">제목을 입력해주세요</div>
			</div>
			<textarea name="f_content" class="form-control contents" id="summernote" required>${updateForm.f_content}</textarea>
			...
			<div class="col-12 pt-4 d-grid gap-2 d-md-flex">
				...
				<button class="btn btn-warning me-md-3 reset" type="reset">다시 작성</button>
				<button class="btn btn-primary ms-md-auto p-2 mt-3 mt-md-0 " type="submit">수정 완료</button>
			</div>
		</form>
	</main>
